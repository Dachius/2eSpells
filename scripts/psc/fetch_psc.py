#!/usr/bin/env python3
"""Scrape the Priest's Spell Compendium spell list and cache every spell page.

Adapted from ../wsc/fetch_wsc.py. Self-contained: extracts the spell titles
from the list page(s), batch-fetches the wikitext of all spell pages via the
MediaWiki API, resolves redirects, and falls back to a "The <name>" title /
search for any red-linked pages.

Writes (next to this script):
    titles.json     -- [{title, name, listLevel}, ...]
    raw_pages.json  -- {"pages": {finalTitle: wikitext}, "norm_map": {from: to}}

Then run parse_psc.py to produce psc_full.json.
"""
...
LIST_PAGES = ["Priest's Spell Compendium Spell List A-Z",
              "Priest's Spell Compendium Spell List S-Z"]
...
def main():
    ...
    for lp in LIST_PAGES:
        print(f"Fetching list page: {lp}", flush=True)
        lst = api_get({"action": "parse", "page": lp,
                       "prop": "wikitext", "format": "json"})
        if "parse" not in lst:
            print(f"  (missing: {lst.get('error', {}).get('info', '?')})", flush=True)
            continue
        for e in extract_titles(lst["parse"]["wikitext"]["*"]):
            byt.setdefault(e["title"], e)
    titles = list(byt.values())
