#!/usr/bin/env python3
"""Parse cached PSC priest-spell wikitext into JSON.

Adapted from ../wsc/parse_wsc.py. Outputs two files:
  ../../json/psc.json  -- flat records matching the existing cleric.json schema
                          (same 17 keys incl. spheres, school normalised to the
                          8 canonical schools, source = "PSC").
  ./psc_full.json      -- rich records (multi-school, sub-schools, spheres,
                          reversibility, original sources, rarity, C&T fields).

Run from this directory after fetch_psc.py has produced raw_pages.json + titles.json:
    python3 parse_psc.py
"""
import json, re, os
...
def parse_sources(raw_source):
    ...
    for line in s.split("\n"):
        ...
        if not t:
            continue
        if re.search(r"priest'?s spell compendium", t, re.I):
            continue
...
def parse_spheres(raw_sphere):
    s = re.sub(r"\{\{\s*br\s*\}\}", "\n", raw_sphere or "", flags=re.I)
    s = strip_markup(s)
    out = []
...
def build(entry):
    ...
    spheres = parse_spheres(p.get("sphere", ""))
...
def main():
    ...
    out_flat = os.path.join(REPO, "json", "psc.json")
    out_rich = os.path.join(HERE, "psc_full.json")
